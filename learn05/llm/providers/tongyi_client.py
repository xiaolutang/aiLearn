...
class TongyiLLMClient(UnifiedLLMInterface):
    """通义千问LLM客户端"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(LLMProvider.TONGYI, config)
        
        # 配置参数
        self.api_key = (self.config.get('api_key') or 
                       os.environ.get('DASHSCOPE_API_KEY') or 
                       os.environ.get('TONGYI_API_KEY') or 
                       os.environ.get('TONG_YI_API_KEY'))
        self.base_url = self.config.get('base_url') or os.environ.get('TONGYI_BASE_URL', 
            'https://dashscope.aliyuncs.com/compatible-mode/v1')
        self.default_model = self.config.get('default_model') or os.environ.get('TONGYI_MODEL', 'qwen-plus')
        ...
        # 模型配置
        self.model_configs = {
            'qwen-turbo': {'max_tokens': 8000, 'context_length': 8000},
            'qwen-plus': {'max_tokens': 32000, 'context_length': 32000},
            'qwen-max': {'max_tokens': 8000, 'context_length': 8000},
            'qwen-max-longcontext': {'max_tokens': 28000, 'context_length': 28000}
        }
    ...
    async def _call_http_api(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            raise TimeoutException("请求超时", LLMProvider.TONGYI)
        except aiohttp.ClientError as e:
            raise NetworkException(f"网络错误: {e}", LLMProvider.TONGYI)
    ...
    async def _stream_http_api(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            raise TimeoutException("流式请求超时", LLMProvider.TONGYI)
        except aiohttp.ClientError as e:
            raise NetworkException(f"流式网络错误: {e}", LLMProvider.TONGYI)
        ...
        if self.rate_limit['request_count'] >= self.rate_limit['requests_per_minute']:
            ...
            if wait_time > 0:
                raise RateLimitException(f"请求频率超限，需等待{wait_time:.1f}秒", 
                                       LLMProvider.TONGYI, int(wait_time))
    ...
    def _handle_api_error(self, error: Exception):
        ...
        if 'unauthorized' in error_str or 'invalid api key' in error_str:
            raise LLMAuthenticationException("API密钥无效", LLMProvider.TONGYI)
        elif 'rate limit' in error_str:
            raise LLMRateLimitException("请求频率超限", LLMProvider.TONGYI)
        elif 'quota' in error_str or 'insufficient' in error_str:
            raise LLMQuotaExceededException("配额不足", LLMProvider.TONGYI)
        elif 'timeout' in error_str:
            raise LLMTimeoutException("请求超时", LLMProvider.TONGYI)
        elif 'model' in error_str and 'not found' in error_str:
            raise LLMModelException("模型不存在", LLMProvider.TONGYI)
        else:
            raise LLMNetworkException(f"API调用失败: {error}", LLMProvider.TONGYI)
    
    def _handle_http_error(self, status_code: int, error_text: str):
        """处理HTTP错误"""
        if status_code == 401:
            raise LLMAuthenticationException("认证失败", LLMProvider.TONGYI)
            ...
            raise LLMNetworkException(f"服务器错误: {status_code}", LLMProvider.TONGYI)
        else:
            raise LLMNetworkException(f"HTTP错误 {status_code}: {error_text}", LLMProvider.TONGYI)